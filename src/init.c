...
int editorSyntaxToColor(int hl) {
  switch (hl) {
  case HL_MLCOMMENT:
  case HL_COMMENT: return colors.commentColor; 
  case HL_FUNC: return colors.funcColor;
  case HL_KEYWORD1: return colors.keyword1Color;
  case HL_KEYWORD2: return colors.keyword2Color;
  case HL_STRING: return colors.stringColor;
  case HL_NUMBER: return colors.numberColor;
  case HL_MATCH: return colors.matchColor;
  default: return colors.normalColor;
  }
}
...
int parseInitFile() 
{
  ...
  colors.normalColor = AS_NUMBER(interpret("print ((normalColor == nil) ? 7 :  normalColor);"));
